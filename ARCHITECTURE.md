...
- **`src/classes/`** — declarative type-class instance tables (`Plus`, `Indexable`; `HasProp`, the constraint a property read on a value of not-yet-known type poses, is structural and has no table). Adding an instance is a one-line change here; the constraint solver in `src/builtins/` consults it.
