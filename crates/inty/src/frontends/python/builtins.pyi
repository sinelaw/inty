...
# primitives: `int` -> Int, `float` -> Number, `str` -> String, `bool` -> Boolean,
...
def open(path: object, mode: str = ..., encoding: str = ...) -> object: ...
...
def sorted(xs: list[T], key: object = ..., reverse: bool = ...) -> list[T]: ...
def reversed(xs: list[T]) -> list[T]: ...
def any(xs: list[T]) -> bool: ...
def all(xs: list[T]) -> bool: ...
# Over the element type, so a `list[int]` sums to an `int`.
def sum(xs: list[T], start: T = ...) -> T: ...
