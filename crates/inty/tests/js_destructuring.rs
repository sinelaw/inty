...
fn array_destructuring_assignment() {
    ...
    assert!(r.is_ok(), "array destructuring assignment should check: {:?}", r);
}

#[test]
fn object_destructuring_assignment_parenthesised() {
    let r = check("let a = 0;\n({a} = {a: 5});\nlet b = a + 1;\n");
    assert!(r.is_ok(), "object destructuring assignment should check: {:?}", r);
}

#[test]
fn array_destructuring_with_rest() {
    let r = check("let h = 0, t = [0];\n[h, ...t] = [1, 2, 3];\nlet z = h + t[0];\n");
    assert!(r.is_ok(), "rest destructuring assignment should check: {:?}", r);
}
...
fn destructuring_assignment_enforces_types() {
    ...
    let r = check("let n = 0;\nlet s = \"\";\n[n, s] = [1, 2];\ns.toUpperCase();\n");
    assert!(r.is_err(), "type mismatch through destructuring should be caught");
}
...
fn nested_destructuring_assignment() {
    ...
    assert!(r.is_ok(), "nested destructuring assignment should check: {:?}", r);
}
...
fn invalid_assignment_target_still_rejected() {
    ...
    assert!(r.is_err(), "assigning to a call result must still be rejected");
}
